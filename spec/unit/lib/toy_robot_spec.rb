...
RSpec.describe ToyRobot do
  ...
  context 'when placed at the origin facing NORTH' do
    ...
    context 'when moving' do
      before do
        allow(grid).to receive(:valid_position?).with(0, 1).and_return(true)
        allow(grid).to receive(:valid_position?).with(1, 0).and_return(true)
        allow(grid).to receive(:valid_position?).with(0, 0).and_return(true)
      end
    ...
    context 'when placing at different coordinates' do
      it 'is placed at (4,4) facing SOUTH' do
        allow(grid).to receive(:valid_position?).with(4, 4).and_return(true)
    ...
    context 'when given invalid moves' do
      before do
        allow(grid).to receive(:valid_position?).with(-1, 0).and_return(false)
        allow(grid).to receive(:valid_position?).with(0, -1).and_return(false)
      end
  ...
  context 'when not placed' do
    context 'when given commands' do
